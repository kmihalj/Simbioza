...
return new class implements ReversibleMigrationInterface {
    /**
     * HR: Kreira prenosivu shemu osobnih postavki, praćenja i odgođenih dostava.
     * EN: Creates the portable personal-preference, follow, and deferred-delivery schema.
     */
    public function up(Database $db): void
    {
        ...
        if (!$schema->hasTable(ModuleSimbiozaUser::TABLE_FOLLOWS)) {
            $schema->create(ModuleSimbiozaUser::TABLE_FOLLOWS, static function (Blueprint $table): void {
                ...
                $table->timestamps();
                $table->unique(['user_id', 'target_type', 'target_id'], 'simbioza_user_follow_target_uq');
                $table->index(['target_type', 'target_id', 'user_id'], 'simbioza_user_follow_subscribers_idx');
            });
        }
        ...
        if (!$schema->hasTable(ModuleSimbiozaUser::TABLE_PENDING_DELIVERIES)) {
            $schema->create(ModuleSimbiozaUser::TABLE_PENDING_DELIVERIES, static function (Blueprint $table): void {
                $table->id();
                $table->string('uuid', 36)->unique();
                $table->bigInteger('user_id')->unsigned()->index();
                ...
                $table->timestamps();
                $table->unique(['user_id', 'dedup_key', 'delivered_at'], 'simbioza_user_pending_dedup_uq');
                $table->index(['user_id', 'delivered_at', 'deliver_after'], 'simbioza_user_pending_due_idx');
            });
        }
    }

    /** HR: Uklanja tablice modula obrnutim redom. EN: Drops module tables in reverse order. */
