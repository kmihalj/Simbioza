...
All 56 scenarios cover every module shipped by Simbioza. They exercise public
...
| Auth | Guest redirect, administrator and regular-user authorization, local login/logout, profile and notification preference updates, reversible password change, one-time personal-space provisioning, profile link, guest concealment, administrator policy screen, group/user CRUD, memberships, ETags, safe output, audit records, and cleanup. |
